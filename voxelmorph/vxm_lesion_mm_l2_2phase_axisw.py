...
Axial/Axis-Weighted Update:
- Option: --lesion-w-mm wx wy wz
  -> Gewichtung pro Achse im mm-Raum für den Lesion-Loss
  -> Reporting-Metrik lesion_err_mm bleibt UNGEWICHTETE euklidische Distanz in mm (vergleichbar!)

Diagnostics Update (AXIAL LOSS sichtbar):
- Loggt zusätzlich Achsen-spezifische "Loss-Anteile" (unweighted & weighted) + RMSE pro Achse:
    * lesion_l2_unw_{x,y,z}      = mean(di^2)
    * lesion_l2_w_{x,y,z}        = mean(wi * di^2)   (das ist dein "axial weighted loss" für z)
    * lesion_rmse_{x,y,z}_mm     = sqrt(mean(di^2))  (interpretierbar in mm)
- Diese Werte werden geloggt in:
    * stdout pro Epoch
    * TensorBoard
    * Weights & Biases
...
- Checkpoints speichern zusätzlich val_lesion_err_mm + val_lesion_mae_xyz_mm + lesion_w_mm
...
def parse_args() -> argparse.Namespace:
    ...
    # Axis-weighted lesion loss in mm (wx wy wz). Default: isotropic.
    # NOTE: lesion_err_mm metric stays UNWEIGHTED Euclidean distance in mm.
    p.add_argument("--lesion-w-mm", type=float, nargs=3, default=[1.0, 1.0, 1.0],
...
def main():
    ...
    # spacing logging flag
    spacing_logged = False

    def lesion_loss_and_err(pred_mm: torch.Tensor, gt_mm: torch.Tensor, eps: float):
        """
        Returns:
          lesion_loss_raw: depends on lesion_loss_mode and axis weights (mm space)
          lesion_err_mm_mean: UNWEIGHTED mean euclidean mm distance (reporting metric)
          mae_xyz_mm: (3,) mean absolute error per axis in mm (x,y,z)

          NEW:
          l2_unw_xyz: (3,) mean(di^2) per axis (unweighted)
          l2_w_xyz:   (3,) mean(wi * di^2) per axis (weighted)  -> axial weighted loss is l2_w_xyz[2]
          rmse_unw_xyz_mm: (3,) sqrt(mean(di^2)) per axis (unweighted RMSE, mm)
        """
        diff = pred_mm - gt_mm                          # (B,3)
        absdiff = diff.abs()
        mae_xyz = absdiff.mean(dim=0)                   # (3,)

        # reporting metric (keep comparable to earlier runs)
        ...
        diff2 = diff * diff                             # (B,3)
        l2_unw_xyz = diff2.mean(dim=0)                  # (3,) mean(di^2)
        l2_w_xyz = (diff2 * w).mean(dim=0)              # (3,) mean(wi*di^2)
        rmse_unw_xyz = torch.sqrt(l2_unw_xyz + eps)     # (3,) mm

        # --- actual lesion supervision loss (scalar) ---
        if args.lesion_loss_mode == "l2":
            loss_raw = torch.sqrt((w * diff2).sum(dim=-1) + eps).mean()
        elif args.lesion_loss_mode == "charbonnier":
            loss_raw = torch.sqrt((w * diff2).sum(dim=-1) + eps).mean()
        else:  # smoothl1
            sw = torch.sqrt(w + 1e-12)
            loss_raw = smoothl1_fn(pred_mm * sw, gt_mm * sw)
    ...
    def run_epoch(loader, train: bool, params: Dict[str, float], epoch: int) -> Dict[str, float]:
        ...
        with ctx:
            for mv, fx, mv_les_vox, fx_les_mm, fx_aff in loader:
                ...
                # ---- log spacing once (first val batch) ----
                if (not train) and (not spacing_logged):
                    A = fx_aff[0, :3, :3]
                    sx = float(torch.linalg.norm(A[:, 0]).item())
                    sy = float(torch.linalg.norm(A[:, 1]).item())
                    sz = float(torch.linalg.norm(A[:, 2]).item())
                    print(f"[Spacing] fx voxel size approx (mm): sx={sx:.4f}, sy={sy:.4f}, sz={sz:.4f}")
                    wandb.log(
                        {"data/fx_spacing_x_mm": sx, "data/fx_spacing_y_mm": sy, "data/fx_spacing_z_mm": sz},
                        step=epoch,
                    )
                    wandb.run.summary["fx_spacing_x_mm"] = sx
                    wandb.run.summary["fx_spacing_y_mm"] = sy
                    wandb.run.summary["fx_spacing_z_mm"] = sz
                    spacing_logged = True
                ...
                with torch.amp.autocast(autocast_device, enabled=use_autocast):
                    ...
                    lesion_raw, lesion_err_mean, mae_xyz, l2_unw_xyz, l2_w_xyz, rmse_xyz = lesion_loss_and_err(
        ...
        wandb.log(
            {
                ...
                "metric/train_lesion_mae_x_mm": train_avg["lesion_mae_x_mm"],
                "metric/train_lesion_mae_y_mm": train_avg["lesion_mae_y_mm"],
                "metric/train_lesion_mae_z_mm": train_avg["lesion_mae_z_mm"],
