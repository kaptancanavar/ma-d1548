...
class VibeDatasetPairwise3D(Dataset):
    ...
    def _create_lists(self):
        ...
        for sample_dir in self.sample_dirs:
            ...
            n_frames= shape[1]
            self.fully_data_list.append(np.abs(np.load(full_path)))
            self.undersampled_data_list.append(np.abs(np.load(full_path).transpose(1,0,2,3)))
            ...
            for us in self.undersampling_list:
                ...
                self.fully_data_list.append(np.abs(np.load(full_path).transpose(1,0,2,3)))
                self.undersampled_data_list.append(np.abs(np.load(undersampled_path).transpose(1,0,2,3)))
                n = self.fill_list_train(n, n_frames, index_us, index_fully)
                index_us += 1
                index_fully += 1
    ...
    def __getitem__(self, idx):
        ...
        fully = self.fully_data_list[entry['fully_idx']]

        t1, t2 = entry['t1'], entry['t2']
        idx1, idx2 = self.get_neighboring_frames(undersampled.shape[0], t2)

        ref_fully = self.normalize(fully[t1])
        mov_fully = self.normalize(fully[t2])
    ...
    def center_crop_or_pad(self, img, crop_d=128, crop_h=64, crop_w=128):
        d, h, w = img.shape[-3:]
