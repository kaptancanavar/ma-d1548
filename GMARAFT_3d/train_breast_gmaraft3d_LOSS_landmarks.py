...
class BreastPairDataset(Dataset):
    ...
    def _pick_point_mm(self, pts: List[Tuple[str, np.ndarray]]) -> np.ndarray:
        ...
        for name_l, xyz in pts:
            for key in self.lm_names:
                if key and key in name_l:
                    return xyz
...
def resolve_lm_names(preset: str, names_csv: str) -> List[str]:
    """Return a list of accepted point-name keys (lowercased, substring-matched)."""
    ...
    if preset == "lesion":
        return ["lesion", "läsion", "laesion", "tumor", "mass"]
    if preset == "tip":
        return ["needletip", "needle tip", "needle-tip", "tip", "nadelspitze", "spitze", "needle"]
