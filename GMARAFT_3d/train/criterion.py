...
import torch
from .losses import *
from .warp import warp_torch,warp_3d_torch, warp_3d_batch_bspline
from .stn import SpatialTransformer3D
...
class Criterion(CriterionBase):
    ...
    def forward(self, y_pred, y_true):
        ...
        total_loss = 0
        flow_preds = y_pred
        ref, mov = y_true  # Already on CUDA from trainer
            ...
            for i, (i_weight, i_flow) in enumerate(zip(i_weights, flow_in_loop)):
                if loss_name == 'photometric':
                    if len(ref.shape)==4:
                        ...
                        warped = warp_3d_batch_bspline(mov, i_flow)
                        
                    i_loss = loss_term(ref, warped)
                elif loss_name == 'smooth':
        ...
        if self.diffeomorphic:
            # Compute backward flow by applying forward flow to itself and negating
            flow_predictions_backward = [-1 * self.stn(flow, flow) for flow in flow_preds]
            
            # Add diffeomorphic constraint: forward + backward should be identity (zero displacement)
