...
def normalize(vol_t: torch.Tensor) -> torch.Tensor:
    ...
    Per-frame min-max to [-1, 1] (MUST match training normalization!)
    """
    if vol_t.ndim == 3:
    ...
    return 2.0 * (vol_t - vmin) / (vmax - vmin + 1e-8) - 1.0


def _normalize01_single(vol_3d: torch.Tensor) -> torch.Tensor:
    # (D,H,W) -> [0,1] (only used for bbox computation, not network input)
    vmin = vol_3d.min()
    vmax = vol_3d.max()
...
def main():
    ...
    img_path = os.path.join(root_dir, "reconstructed_img_fista_motionC_abs_tv0503.mhd")
    ...
    target_shape = tuple(ds_cfg.get("target_shape", [64, 128, 128]))
    ...
    checkpoint_path = os.path.join(cwd, config["trainer"]["save_dir"], config["name"], "150_final.pth")
    ...
    for i in range(0, num_states):
        ...
        # Predict flow on training grid (exact target_shape, align_corners=True)
        flow_target = predict_flow_on_training_grid(
            model, fixed_cropN, moving_cropN, target_shape=target_shape
        )  # (1,3,Dt,Ht,Wt)

        # Resize flow back to crop size (with vector scaling, align_corners=True)
        dc, hc, wc = moving_crop.shape[1:]
        flow_crop = resize_flow_3d(flow_target, (dc, hc, wc), align_corners=True)  # (1,3,dc,hc,wc)

        # Paste flow into full-FOV
        flow_full = paste_flow((D, H, W), flow_crop, bbox)  # (1,3,D,H,W)

        # Warp full-FOV moving volume
        warped = warp_3d_batch_bspline(moving_full.unsqueeze(0), flow_full)  # (1,1,D,H,W)
        registered_volume[i] = warped.squeeze().detach().cpu().numpy()

        # Save forward field (D,H,W,3)
        # Network outputs (x,y,z), but need to save as (z,y,x) for consistency with bspline grid
        flow_vox_dhwd3 = flow_full[0].permute(1, 2, 3, 0).flip(-1).detach().cpu().numpy()  # Flip (x,y,z)→(z,y,x)
        deformation_fields_fwd[i] = flow_vox_dhwd3

        # Invert forward field with fixed-point (voxel units, in z,y,x order)
        flow_for_inv = torch.from_numpy(flow_vox_dhwd3[None, ...]).to(device).float()  # (1,D,H,W,3) as (z,y,x)
        inv_field = invert_displacement_fixed_point_3d(flow_for_inv, num_iters=40)      # (1,D,H,W,3)
